# -*- coding: utf-8 -*-
"""
Created on Tue Mar  9 19:09:52 2021

@author: G-GRE-GRE050402
"""

###the trigger is now given by the start of the acquisition, all the delay must refer to this
#i.e. 100us pulse 
...
name="\\pulse_triggerdata_transfer.seqc"
...
def changetacquisition(filename,A):
    lines=open(filename).read().splitlines()
    with open(filename,'r+') as f:
         f.truncate(0)
         lines[48]='const gate_window_sec =' +str(A)+';'
         for i in range(len(lines)):
             f.write(lines[i]+'\n')   
             #f.writelines()
    f.close()   
...
delay=tramp+t11*3-burst_time
##set mw gen
# mwgen.power(-10)
mwgen.burstwidth(burst_time)
mwgen.trigdelay(tramp+t11*3-burst_time)
# mwgen.burstwidth(burst_time)
# mwgen.frequency(?)
...
changetacquisition(namefile,tacquisition)
...
# trigger_mode() 
tc=0.5
acquisition_duration =20e-6  # length of time to record (s)
# repetitions = 10            # number of repetitions for the averaging
points = 36 # number of points in the acquisition window (min=2)
...
#                   ['dataAcquisitionModule/type', 2],      
                   ...
                   ##careful
                   ['dataAcquisitionModule/delay', 3e-6], #0                                       # trigger delay (s)
                   ##
...
detuningPoint1 = [-846.4,-832.7]  #TPC #G3,G4
detuningPoint2 = [-847.3,-830.1] 
...
# name=str(Bfield())+'T_detuningvsfREQUENCY_mWpower-20dB_G1'+str(VG1())+'mV_G2'+str(VG2())+'mV_G3'+str(VG3())+'mV_G4'+str(VG4())+'mV_G5'+str(VG5())+'mV_G6'+str(VG6())+'mV'
# a,dataid,c,d=sweep2D(G4detuning,-831.2,-831.4,21,0.05,mwgen.frequency,13.65e9,14e9,701,0.005,phase_trigS)
# # a,dataid,c,d=sweep2D(VG3comp,-847,-845,101,0.05,VG4comp,-831,-833,101,0.02,ph1,ph2)
# plot_by_id(dataid)
# saveplot(name,dataid)
...
# mwgen.on()
# mwgen.power(-10)
# name=str(Bfield())+'T_freqvsdetuning_power-35dBm_OS_G1'+str(VG1())+'mV_G2'+str(VG2())+'mV_G3'+str(VG3())+'mV_G4'+str(VG4())+'mV_G5'+str(VG5())+'mV_G6'+str(VG6())+'mV'
# mwgen.on()
# a,dataid,c,d=sweep2D(mwgen.frequency,16.77e9,16.79e9,101,0.01,G4detuning,-832.5,-831.5,201,0.0001,ph1,ph2)
# plot_by_id(dataid)
# save2plots(name,dataid)
