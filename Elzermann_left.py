# -*- coding: utf-8 -*-
"""
Created on Tue Mar 30 14:24:14 2021
...
# name=str(Bfield())+'T_G3vsG2_N4to5_G1'+str(VG1())+'mV_G2'+str(VG2())+'mV_G3'+str(VG3())+'mV_G3'+str(VG3())+'mV_G2'+str(VG2())+'mV_G6'+str(VG6())+'mV'
# # a,dataid,c,d=sweep2D(Vg2,-1000,-1200,801,0.05,VG3,-760,-860,401,0.01,ph2)
# # plot_by_id(dataid)
# # saveplot(name,dataid)
# VG3(-1100)
# VG6onmin_phD(-1500,-1600,1601,1)
# a,dataid,c,d=sweep2D(Vg2,-1200,-1250,201,0.05,VG3,-700,-800,201,0.01,ph2)
# plot_by_id(dataid)
# saveplot(name,dataid)
...
def Elzouzou_left(path,start,stop,G2f,G3f,average,Npoints, Tempty,Tload, Tread,Vload=-0.1,Vread=0,Vempty=0.1) :
    ...
    points=int(acquisition_duration*ziUhf.daq.getDouble('/dev2010/demods/1/rate'))
    
    
    ziUhf.daq.setDouble('/dev2010/demods/1/timeconstant', efftc)
    ziUhf.daq.setDouble('/dev2010/demods/1/timeconstant', efftc)
    ziUhf.daq.setInt('/dev2010/awgs/0/enable', 1)      # 1 = enable;   0 = disabled
    ...
    ziUhf.daq.sync()
    trigger_setting = [['dataAcquisitionModule/triggernode', '/dev2010/demods/1/sample.TrigAWGTrig3'],  # trigger node is AWG trigger number 3 
...
# g2 = [-1207,-1213,-1218,-1223,-1229,-1234,-1239,-1244,-1249,-1254,-1259,-1265,-1261,-1276,-1281,-1286,-1291,-1296,-1301,-1306,-1311,-1317,-1322,-1327,-1332,-1337,-1342,-1347,-1352,-1357,-1362,-1367,-1377,-1382,-1387,-1392,-1397,-1402,-1407]
# G3 = [ -748, -748, -746, -746, -746, -745,  -745,-745 ,-744, -744,  -744, -743,-743,  -743,-742 ,-742 ,-742,-741  ,-741,  -740,-740 ,-739,-739   ,-739,-738,-738   ,-738,-737,-737   ,-737,-736,-736   ,-735,-735,  -734,-734, -733, -733, -733]


# g2=np.linspace(-1333,-1433,21)
# G3=np.linspace(-779,-773,21)
...
# Bfield(1.3)
G2=np.linspace(-1056,-963,18)

G3=np.linspace(-735,-801,18)




G2=np.linspace(-1028,-963,1)

G3=np.linspace(-752,-799,13)
...
# G2=[-980]
# G3=[-781]
# VG1onmin_phS(-1640,-1660,201,1)
G2=np.linspace(-1003,-1083,16)

G3=np.linspace(-758,-706,16)


G2=np.linspace(-1012,-1083,14)

G3=np.linspace(-753,-706,14)
...
# G3=[-792,-796]
G2=np.linspace(-952,-922,7)


G3=np.linspace(-804,-706,14)
...
elzouzou = True 
# VG1onmin_phS(-1650,-1750,1000,1)

for i,j in zip(G2,G3):
    ...
    if Scan2d == True:
        ...
        # tc=40e-3
        tc=10e-3
        ziUhf.daq.setDouble('/'+uhfRef+'/demods/1/timeconstant',tc)
        ziUhf.daq.setDouble('/'+uhfRef+'/demods/1/rate',2e3) 
        ziUhf.daq.setDouble('/'+uhfRef+'/demods/1/order',3) 
        # ziUhf.daq.setInt('/dev2010/demods/2/enable', 1)
        ziUhf.daq.setInt('/dev2010/demods/1/enable', 1)

        # ziUhf.daq.setInt('/dev2010/sigouts/0/enables/6', 1)
        name=str(Bfield())+'T_G3vsG2_N0to1_G1'+str(VG1())+'mV_G2'+str(VG2())+'mV_G3'+str(VG3())+'mV_G4'+str(VG4())+'mV_g5'+str(VG5())+'mV_G6'+str(VG6())+'mV'
        # c1,data_id,c2,c3=sweep2D(VG2,i-3,i+3,121,tc,VG3,j-4,j+4,161,tc,ph2)
        # c1,data_id,c2,c3=sweep2D(VG2,i-3,i+3,41,tc,VG3,j-4,j+5,51,tc,ph2)
        c1,data_id,c2,c3=sweep2D(VG2,i-2,i+2,81,tc,VG3,j-3,j+3,81,tc,ph2)
    ...
    # Enregistrement du scan précédent sous forme 
        ...
        Field = Bfield_Hon()
        pathh= f'..\exploration\G3=%s_g2=%s'%(str(round(j,3)),str(round(i,3)))
        ...
        ##correct position
        # Vgw5=Vgw5+0.1
        # # print('working_point_G3'+str(VG3())+'_g2'+str(VG2()))
...
# s'assurer qu'il n'y a pas un système a deux niveau qui traine dans le coin. 
    ...
    if time_traces == True:
        ...
        path_ex=register_time_trace(daq,-0.5,0.1,nb_elements,1e-5,2,demod,path3,a)
        
     #Post traitement des gammas rates et de la probabilité d'occupation dans le dot 
    # Threshold2=5
        gammarate=analyse_time_traces(pathh,-0.5,0.1,Threshold,str(round(j,3)),str(round(i,3)))
    ...
    if elzouzou==True :
        ...
        VG3(Vgw4)
        VG2(Vgw5) 
        Elzouzou_left(pathh,-0,-1,Vgw4,Vgw5,1000,41, 40e-6, 150e-6,200e-6)
...
# 628_1.4T_G3vsG2_N0to1_G1-1696.6000000000001mV_G2-974.0799999999999mV_G3-785.0780000000001mV_G4-700.0mV_g5-1450.0mV_G6-1542.05mV


# name=str(Bfield())+'T_G2vsG3_N0to1_G1'+str(VG1())+'mV_G2'+str(VG2())+'mV_G3'+str(VG3())+'mV_G4'+str(VG4())+'mV_G5'+str(VG5())+'mV_G6'+str(VG6())+'mV'

# # a,dataid,c,d=sweep2D(VG2,-960,-1100,601,0.05,VG3,-750,-900,601,0.01,ph2)

# a,dataid,c,d=sweep2D(VG2,-960,-900,121,0.05,VG3,-800,-850,101,0.01,ph2)
# plot_by_id(dataid)
# saveplot(name,dataid)
